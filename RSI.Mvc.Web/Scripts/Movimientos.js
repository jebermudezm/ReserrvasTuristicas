function ajaxGetMovimientos(url) {
    $("#modalRSI").html("");
    $.ajax({
        type: 'GET',
        url: url,
...
function ajaxPostCrearMovimientos(url) {
    $.ajax({
        type: 'POST',
        url: url,
        data: $("#crearMovimientosForm").serialize(),
        dataType: 'json',
        cache: false,
        success: function (result, status, xhr) {
            if (typeof result === 'undefined') {
                $("#modalRSI").modal('toggle');
                $("#modalRSI").html('');
                $("#gridDatosMovimientos").data("kendoGrid").dataSource.read();
            }
            else {
                if (result.hasOwnProperty('Success')) {
    ...
    $.ajax({
        ...
        success: function (result, status, xhr) {
            if (typeof result === 'undefined') {
                $("#modalRSI").modal('toggle');
                $("#modalRSI").html('');
                var grids = document.querySelectorAll('[id^="gridDatosMovimientos"]');
...
function ajaxPostHabilitarMovimientos(url) {
    $.ajax({
        type: 'POST',
        url: url,
        data: {
...
function ajaxPostEditarMovimientos(url) {
    $.ajax({
        type: 'POST',
        url: url,
        data: $("#editarMovimientosForm").serialize(),
        dataType: 'json',
        cache: false,
        success: function (result, status, xhr) {
            if (typeof result === 'undefined') {
...
function ajaxPostMovimientosDetalle(url, nombreFormulario) {
    $.ajax({
        type: 'POST',
        url: url,
...
function ajaxPostEliminarMovimientos(url) {
    $.ajax({
        type: 'POST',
        url: url,
        data: $("#EliminarMovimientosForm").serialize(),
        dataType: 'json',
        cache: false,
        success: function (result, status, xhr) {
            if (typeof result === 'undefined') {
